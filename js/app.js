...
const createHoverEfecct = () => {
    globe
    .selectAll(".country")
    .on("mouseover", function(e, d){
        const {formal_en, economy} = d.properties;
        infoPanel.html(`<h1>${formal_en}</h1><hr><p>${economy}</p><hr><p>Power By <a style="color: rgb(71, 174, 214)" href="https://www.facebook.com/SoriaDev" target="_blank">Soria-Developer 🐼</a></p>`);
        globe.selectAll(".country").style("fill", "#33415c").style("stroke", "#060a0f");
        select(this).style("fill", "#6ea9ff").style("stroke", "white").style("cursor", "pointer");
    })
};
...
const createDraggingEvents = () => {
    ...
    .on("mousemove", e => {
        if(isMouseDown){
            ...
            selectAll(".graticulate").attr("d", path(graticule()));
        }
